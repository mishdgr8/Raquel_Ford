...
export function ArticleRenderer({ blocks, html }: ArticleRendererProps) {
    ...
    if (isHtmlMeaningful) {
        ...
        // Regex to find Image Gallery generated by Tiptap
        const galleryRegex = /<div[^>]*data-type="image-gallery"([^>]*)>[\s\S]*?<\/div>(\s*<\/div>)?/gi;

        // Combine regexes to split the HTML properly
        const combinedRegex = new RegExp(`${twitterRegex.source}|${galleryRegex.source}`, 'gi');
        ...
        while ((match = combinedRegex.exec(html)) !== null) {
            ...
            const isTwitter = fullMatch.includes('twitter-tweet') || fullMatch.includes('twitter.com') || fullMatch.includes('x.com');
            ...
            if (isTwitter) {
                ...
                // It's a Gallery match
                // We use another regex to find the attributes inside the first capture group of the gallery match
                ...
                if (imagesAttr) {
                    try {
                        ...
                        console.error("Failed to parse gallery images from HTML", e);
                        parts.push(
                            <div
                                key={`html-fallback-${match.index}`}
                                className={styles.container}
                    ...
                    // Fallback if imagesAttr is not found
                    parts.push(
                        <div
                            key={`html-no-attr-${match.index}`}
                            className={styles.container}
                            dangerouslySetInnerHTML={{ __html: fullMatch }}
                        />
                    );
